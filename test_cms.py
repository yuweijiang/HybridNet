...
from utils.gt_caps_dataloader import VideoDataset_mix40 as VideoDataset
from model.transformer.Translator import translate_batch_cap2cms
...
def test(loader, model, opt, cap_vocab, cms_vocab):
    ...
    for batch_id, raw_data in enumerate(loader):
        ...
        for iterate_id in range(len(raw_data)):
            ...
            i3d = raw_data[iterate_id][0]['i3d'].cuda().unsqueeze(0)  # 10*1024
            audio = raw_data[iterate_id][0]['audio'].cuda().unsqueeze(0)  # 10*1024
            ...
            with torch.no_grad():
                ...
                cms_batch_hyp = translate_batch_cap2cms(model, fc_feats, cap_labels, i3d, audio, opt)
            ...
            for random_id in range(cap_labels.shape[0]):
                ...
                json_dict[video_ids+'_'+str(eval_id)] = {'cap': cap_gt, 'gt':cmses, 'pred':cms}
    ...
    with open("./json/{}.json".format(opt['load_checkpoint'].split('/')[-1].split('.')[0]+'_'+opt['cms']), "w") as f:
